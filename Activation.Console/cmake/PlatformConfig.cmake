...
elseif(UNIX)
    ...
    # If we are on WSL/dual boot, ignore Windows library locations so find_package
    # picks native Unix toolchains instead of mingw archives.
    set(_windows_prefixes "")
    if(EXISTS "/mnt/c/Program Files")
        list(APPEND _windows_prefixes "/mnt/c/Program Files")
    endif()
    if(EXISTS "/mnt/c/Program Files (x86)")
        list(APPEND _windows_prefixes "/mnt/c/Program Files (x86)")
    endif()
    foreach(_prefix_path IN LISTS _windows_prefixes)
        list(APPEND CMAKE_IGNORE_PATH "${_prefix_path}")
        list(APPEND CMAKE_IGNORE_PREFIX_PATH "${_prefix_path}")
    endforeach()
endif()
...
if(UNIX AND NOT APPLE)
    ...
    if(CMAKE_LIBRARY_ARCHITECTURE)
        set(LINUX_LIB_PREFIX "/usr/lib/${CMAKE_LIBRARY_ARCHITECTURE}")
    else()
        set(LINUX_LIB_PREFIX "/usr/lib")
    endif()
    ...
    # Ensure common third-party packages resolve to native Linux locations when running under WSL.
    if(EXISTS "${LINUX_LIB_PREFIX}")
        if(NOT DEFINED OPENSSL_ROOT_DIR OR OPENSSL_ROOT_DIR STREQUAL "")
            set(OPENSSL_ROOT_DIR "${LINUX_LIB_PREFIX}" CACHE PATH "Preferred OpenSSL root on Linux")
        endif()
        if(NOT DEFINED ZLIB_ROOT OR ZLIB_ROOT STREQUAL "")
            set(ZLIB_ROOT "${LINUX_LIB_PREFIX}" CACHE PATH "Preferred zlib root on Linux")
        endif()
    endif()
    if(EXISTS "/usr/include")
        if(NOT DEFINED OPENSSL_INCLUDE_DIR OR OPENSSL_INCLUDE_DIR STREQUAL "")
            set(OPENSSL_INCLUDE_DIR "/usr/include" CACHE PATH "Preferred OpenSSL include directory on Linux")
        endif()
        if(NOT DEFINED ZLIB_INCLUDE_DIR OR ZLIB_INCLUDE_DIR STREQUAL "")
            set(ZLIB_INCLUDE_DIR "/usr/include" CACHE PATH "Preferred zlib include directory on Linux")
        endif()
    endif()
endif()
